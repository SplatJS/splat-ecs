...
function createSprites(textureManifests, images, gl, spriteList) {
	object.values(textureManifests).reduce(function(sprites, textureManifest) {
		var image = textureManifest.meta.image;
		var t = textures.fromImage(gl, images.get(image));
		var s = textures.toSprites(gl, t, textureManifest);
...
function Game(canvas, customRequire) {
	...
	var textureManifests = loadTextureManifests(customRequire);
	var imageManifest = buildImageManifest(textureManifests);
	this.sprites = {};
	var images = new ImageLoader(function() {
		createSprites(textureManifests, images, this.context, this.sprites);
	}.bind(this));

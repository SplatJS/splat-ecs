...
Game.prototype.makeScene = function(name, sceneData, sceneArgs) {
	...
	} else if (sceneData.onEnter.constructor === Array) {
        scene.onEnter = [];
        for (var i = 0; i < sceneData.onEnter.length; i++) {
            var enterScript = this.require(sceneData.onEnter[i]);
            if (typeof enterScript === "function") {
                enterScript = enterScript.bind(scene, data);
            }
            scene.onEnter[i] = enterScript;
        }
	...
	} else if (sceneData.onExit.constructor === Array) {
        scene.onExit = [];
        for (var i = 0; i < sceneData.onExit.length; i++) {
            var exitScript = this.require(sceneData.onExit[i]);
            if (typeof exitScript === "function") {
                exitScript = exitScript.bind(scene, data);
            }
            scene.onExit[i] = exitScript;
        }

...
function SoundLoader(manifest) {
  ...
  this.sounds = loadAssets(manifest, loadSound.bind(undefined, this.context), function(err) {
    if (err) {
      console.error(err);
    }
  });
}
SoundLoader.prototype.bytesLoaded = function() {
  return Object.keys(this.sounds).reduce(function(accum, key) {
    var sound = this.sounds[key];
    return accum + sound.loaded;
  }.bind(this), 0);
};
SoundLoader.prototype.totalBytes = function() {
  return Object.keys(this.sounds).reduce(function(accum, key) {
    var sound = this.sounds[key];
    return accum + sound.total;
  }.bind(this), 0);
};

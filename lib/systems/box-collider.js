...
// function removeEntity(entity, oldKeys, entities, spatialHash, collisions) {
...
module.exports = function(ecs, data) {
	...
	// data.entities.onRemoveComponent("collisions", function(entity, component, collisions) { // eslint-disable-line no-unused-vars
	// 	var collisionKeys = data.entities.get(entity, "collisionKeys");
	// 	removeEntity(entity, collisionKeys || [], data.entities, spatialHash, collisions);
	// });
	// ecs.addEach(function boxCollider(entity, elapsed) { // eslint-disable-line no-unused-vars
	// 	perf.start();
	// 	var collisionKeys = data.entities.get(entity, "collisionKeys");
	...
	// 	}
	// }, "boxCollider");
	ecs.add(function boxColliderFancy(entities, elapsed) { // eslint-disable-line no-unused-vars
		// if (window.performance.now() - started > 1000) {
		// 	perf.log();
		// 	perf.reset();
		// 	started = window.performance.now();
		// }
		var ids = data.entities.find("boxCollider");
		var boxes = ids.map(function(entity) {
			data.entities.get(entity, "collisions").length = 0;
			var position = data.entities.get(entity, "position");
			var size = data.entities.get(entity, "size");
			return [position.x, position.y, position.x + size.width, position.y + size.height];
		});
		boxIntersect(boxes, function(a, b) {
			var idA = ids[a];
			var idB = ids[b];

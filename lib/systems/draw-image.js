...
function drawEntity(game, entity, context) {
	...
	var dw = imageComponent.destinationWidth || game.entities.get(entity, "size").width || image.width;
	if (dw === 0) {
		console.warn("destinationWidth is 0, image would be invisible for entity", entity, game.entities.get(entity, "name"));
	}
	var dh = imageComponent.destinationHeight || game.entities.get(entity, "size").height || image.height;
	if (dh === 0) {

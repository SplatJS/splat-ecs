...
function drawEntity(game, entity, pMatrix, mvMatrix, camera) {
	...
	drawSprite(game.context, game.renderer.shaderProgram, image, pMatrix, mvMatrix, dw, dh);
...
function drawSprite(gl, shaderProgram, sprite, pMatrix, mvMatrix, dw, dh) {
	if (!sprite) {
		return;
	}

	var vertexCoords = buildBuffer(gl, 3, makeRectangleCoords(dw, dh));
	gl.bindBuffer(gl.ARRAY_BUFFER, vertexCoords);
	gl.vertexAttribPointer(shaderProgram.vertexPositionAttribute, vertexCoords.itemSize, gl.FLOAT, false, 0, 0);
	...
	gl.drawArrays(gl.TRIANGLE_STRIP, 0, vertexCoords.numItems);
}

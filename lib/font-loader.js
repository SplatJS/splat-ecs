import "../vendor/FontLoader.js";
import platform from "./platform";

function buildFontFaceRule(family, urls) {
  const eot = urls["embedded-opentype"];
  const woff = urls.woff;
  const ttf = urls.truetype;
  const svg = urls.svg;
...
function createCssFontFaces(fontFamilies) {
  ...
  for (let family in fontFamilies) {
    if (Object.prototype.hasOwnProperty.call(fontFamilies, family)) {

 ...
 */
export default function importTilemap(file, entities, images) {
...
function collectionOfImagesToImage(tileset, imageComponents, images) {
  var keys = Object.keys(tileset.tiles).map(function (id) {
    return parseInt(id);
  });
...
function makeTiles(file, z, entities, imageComponents) {
  ...
  if (layer.compression === "zlib") {
    ...
    data = new window.Uint32Array(inflated.buffer);
  }
  for (var i = 0; i < data.length; i++) {
    ...
    var x = gridX * file.tilewidth + (layer.offsetx || 0);
    var y =
      (gridY + 1) * file.tileheight - image.sourceHeight + (layer.offsety || 0);
    var entity = makeTile(
      { x: x, y: y, z: z },
      { x: gridX, y: gridY, z: z },
      image,
    ...
    setComponentsFromProperties(
      entity,
      imageComponents[tile].properties,
      entities
    );
...
function makeObject(layer, object, z, entities, imageComponents) {
  ...
  position.x = object.x + (layer.offsetx || 0);
  position.y = object.y + (layer.offsety || 0);
  position.z = z;
  ...
  if (object.gid !== undefined) {
    ...
    setComponentsFromProperties(
      entity,
      imageComponents[object.gid].properties,
      entities
    );
  }
  setComponentsFromProperties(entity, layer.properties, entities);
...
function setComponentsFromProperties(entity, properties, entities) {
  ...
  Object.keys(properties).forEach(function (key) {
    ...
    setOrAddComponent(entities, entity, key, value);
  });
}

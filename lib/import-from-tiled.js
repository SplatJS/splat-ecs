...
/** Import an orthogonal tilemap from <a href="http://www.mapeditor.org/" target="_blank">Tiled: Map Editor</a> and draw on the screen
...
function ImportTilemap(file, entities) { // eslint-disable-line no-unused-vars
	...
	for (var i = 0; i < file.layers.length; i++) {
		...
		/**
		 * <p><b>Collidable</b>: True or False <br />Cause this object to have a collision component. Currently collisions are handled as a simulation system in the project.</p>
		 * <p><b>Spawn</b>: True or False <br />Creates an entity with a "spawn" boolean component allowing you to use <code>game.entities.find("spawn");</code> to find the spawn point for this map.</p>
		 * <br />All other properties end up in a "tiled_properties" component to be handled on a per-game basis.
		...
		if (layer.type == "objectgroup") {
			for (var j = 0; j < layer.objects.length; j++) {
				...
				entities.set(entity, "tiled_properties", object.properties);

				// Collision box prefabs
				if (object.properties.Collidable) {
					entities.set(entity, "name", "collider");
					entities.set(entity, "collisions", []);
				}

				// Spawn points
				if (object.properties.Spawn) {
					entities.set(entity, "name", "spawn");
	...
	/**
	 * <p><b>Container Entity</b> is created automatically allowing you to use <code>game.entities.find("container");</code> to get a constrainPosition ID</p>

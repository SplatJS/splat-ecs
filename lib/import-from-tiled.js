		...
		if (layer.type == "objectgroup") {
			for (var j = 0; j < layer.objects.length; j++) {
				...
				if (object.properties.Collidable == "True") {
					collider = entities.create();
					entities.set(collider, "name", "collider");
					entities.set(collider, "size", { "width": object.width, "height": object.height });
					entities.set(collider, "position", { "x": object.x, "y": object.y });
				}

				// Spawn points
				if (object.properties.Spawn == "True") {
					var spawn = entities.create();
					entities.set(spawn, "name", "spawn");
					entities.set(spawn, "spawn", true);
					entities.set(spawn, "size", { "width": object.width, "height": object.height });
					entities.set(spawn, "position", { "x": object.x, "y": object.y });
				}
			}
		}
	}
